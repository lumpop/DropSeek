...
1. 支持绑定和上架的商品店铺平台：Shopify, ShopBase，Mshop
...
进入谷歌Chrome应用商店，搜索Dropseek。找到Dropseek插件，选择“添加拓展程序”后安装下载。

![](.gitbook/assets/0.png)

### 启动Dropseek

安装完成Dropseek后，在拓展程序中找到Dropseek点击即可进入登入页面。页面如下：

![](.gitbook/assets/1.png)

### 创建你的Dropseek账号

第一次访问的时候，通过以下步骤进行用户注册：点击“注册账号”->进入注册页面 -> 输入邮箱地址 -> 输入密码 -> 再次输入确认密码 -> 点击“注册”。页面如下：
...
登入成功的用户，首先需要在Dropseek中绑定店铺(目前支持的店铺有Shopify，Mshop和ShopBase)，并将店铺权限授权给应用。用户进入导航栏 -> 进入店铺授权页面 -> 点击“增加店铺”-> 出现店铺授权弹窗 -> 然后选择授权方式。授权方式一共有2种：

* 通过“安装应用”来输入自己的店铺地址，进行店铺的自动授权。方法为点击“增加店铺” -> 点击“安装应用”-> 输入店铺地址 -> 点击“确认”。完成后进入Shopify或ShopBase后台 -> 点击气球图标 -> 点击“Authorize”-> 等待成功后，自动授权完成。

若用户选择使用安装app的形式来自动授权应用，当Token过期时，店铺列表中显示感叹号图标。用户需要重新使用APP授权或者使用手动更新密码的方式来激活店铺。
...
方法一：在用户授权并添加店铺之后，就可以进行商品导入了。用户点击导航栏 -> 进入导入列表页面 -> 在“请输入您的商品详细网址”中输入商品的URL地址 -> 点击“添加”-> 等待，完成商品导入。用户每导入成功一个商品，消耗1点数的Seek积分（积分不足需要充值。导入店匠或速卖通的产品暂时不扣除积分）。

![](.gitbook/assets/商品导入.png)

方法二：使用采集插件进行商品导入。用户需要打开受支持的商品平台的店铺，然后在要导入商品详情页中找到小气球图标并点击 -> 点击“Import Product” -> 等待导入成功 -> 跳转到导入日志查看详情

![](.gitbook/assets/商品.png)
...
![](.gitbook/assets/价格.png)

### 选择要上架的商品图片

用户可以查看并选择导入商品的所有产品图片。进入导入列表页面 -> 找到要修改的商品 -> 点击图片 -> 将鼠标移入要编辑的商品图片 -> 选择要上架的商品图片。

![](.gitbook/assets/30.png)

![](.gitbook/assets/选择图片.png)

### 修改导入商品的分类属性选项

用户可以修改已导入商品的属性名称。进入导入列表页面 -> 找到要修改的商品 -> 点击编辑选项 -> 输入要修改的内容 -> 完成修改

![](.gitbook/assets/修改选项.png)
...
用户可以查看当前DropSeek版本的新增加功能，移除功能或优化功能项的说明。点击进入版本说明页面，即可发现并阅读浏览说明。

![](.gitbook/assets/版本说明.png)
